...
static int k1x_gpio_to_irq(struct gpio_chip *chip, unsigned int offset)
{
...
static int k1x_gpio_direction_input(struct gpio_chip *chip, unsigned int offset)
{
	struct k1x_gpio_chip *k1x_chip =
			container_of(chip, struct k1x_gpio_chip, chip);
...
static int k1x_gpio_get(struct gpio_chip *chip, unsigned int offset)
{
	struct k1x_gpio_chip *k1x_chip =
			container_of(chip, struct k1x_gpio_chip, chip);
	struct k1x_gpio_bank *bank =
...
static void k1x_gpio_set(struct gpio_chip *chip, unsigned int offset, int value)
{
	struct k1x_gpio_chip *k1x_chip =
			container_of(chip, struct k1x_gpio_chip, chip);
	struct k1x_gpio_bank *bank =
...
static irqreturn_t k1x_gpio_demux_handler(int irq, void *data)
{
	...
	for (i = 0; i < k1x_chip->nbank; i++) {
		...
		for_each_set_bit(n, &pending, BITS_PER_LONG) {
			irq_num = irq_find_mapping(k1x_chip->domain,
...
static int k1x_gpio_probe(struct platform_device *pdev)
{
	...
	if (devm_request_irq(&pdev->dev, irq,
			     k1x_gpio_demux_handler, 0,
			     k1x_chip->chip.label, k1x_chip)) {
